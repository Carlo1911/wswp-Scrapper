...
def builtwith(url, headers=None, html=None, user_agent='builtwith'):
    ...
    if None in (headers, html):
        try:
            # request = urllib.request.urlopen(url, None, {'User-Agent': user_agent})
            request = Request(url)
    ...
    if html:
        ...
        if type(html).__name__ == 'str':
            html2 = html
        else:
            html2 = html.decode("utf-8")
        metas = dict(re.compile('<meta[^>]*?name=[\'"]([^>]*?)[\'"][^>]*?content=[\'"]([^>]*?)[\'"][^>]*?>', re.IGNORECASE).findall(html2))
        for app_name, app_spec in data['apps'].items():
...
def contains(v, regex):
    ...
    if type(regex).__name__ == 'str':
        ...
        r2 = regex.decode("utf-8")
    return re.compile(r2.split('\\;')[0], flags=re.IGNORECASE).search(v2)
